...
func TestSortedSet(t *testing.T) {
	...
	valswithsores := make([][]byte, 2*len(svals))
	for i := 0; i < len(svals); i++ {
		vals[i] = []byte(svals[i])
		valswithsores[i*2] = []byte(svals[i])
		valswithsores[i*2+1] = []byte(strconv.FormatFloat(ranks[i], 'f', -1, 64))
	...
	for i := 0; i <= 4; i++ {
		data, _ := client.Zrangewithscores("zs", 0, i)
		if !reflect.DeepEqual(data, valswithsores[0:(i+1)*2]) {
			t.Fatal("zrangewithscores failed")
		}
	}
	for i := 0; i <= 4; i++ {
